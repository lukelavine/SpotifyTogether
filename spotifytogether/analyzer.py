...
clfs = [
        MLPClassifier(hidden_layer_sizes=250, max_iter=250),
...
def generatefalsedata(innorm):
    ...
    while i < len(innorm):
        toosim = False
        fakesong = np.random.uniform(0.0, 1.0, size=(11))
        for song in innorm:
            ...
            if cosinesimlarity(song, fakesong) > minsim:
                ...
                break
        if not toosim:
            fakesong = np.append(fakesong, float(-1))
...
def runprog(sp, tracks):
    ...
    features = audiofeatures(sp, tracks)
    ...
    #print(features)
    for n in features:
        realdata.append(np.append(n, float(1)))
    ...
    for i in range(len(realdata)):
        alldata.append(realdata[i][:-1])
        alldata.append(fakedata[i][:-1])
        alllabels.append(realdata[i][-1])
